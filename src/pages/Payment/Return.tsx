...
export default function Return() {
  ...
  const [countdown, setCountdown] = useState(5); // Countdown for the redirection
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        localStorage.setItem("isVip", data.vip);
        ...
        if (axios.isAxiosError(err) && err.response) {
          setMessage(
            err.response.data.message ||
              "Đã xảy ra lỗi từ server khi xác thực giao dịch."
          );
      ...
      case "SUCCESS":
        return (
          ...
          >
            ...
            <Typography
              variant="h5"
              sx={{ fontWeight: "bold", color: "#10b981", textAlign: "center" }}
            >
              {message}
            </Typography>
            {transactionDetails && (
              ...
              >
                <Typography variant="body1" sx={{ textAlign: "left", mb: 1 }}>
                  <span style={{ fontWeight: "bold" }}>Mã giao dịch:</span>{" "}
                  {transactionDetails.txnRef}
                </Typography>
                <Typography variant="body1" sx={{ textAlign: "left", mb: 1 }}>
            ...
            >
              Chuyển về trang chủ trong... ({countdown})
            </Button>
          </Box>
        );
      case "FAILURE":
        return (
          ...
          >
            ...
            <Typography
              variant="h5"
              sx={{ fontWeight: "bold", color: "#ef4444", textAlign: "center" }}
            >
              {message}
            </Typography>
            <Button
